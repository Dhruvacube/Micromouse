...
Directions_Class = namedtuple('Directions','UP RIGHT DOWN LEFT')
Direction = Directions_Class(0,1,2,3)
...
def mapping(maze_array, x, y, degmode, intersections): #see page 9 of the powerpoint to see how each cell is stored as a combinatation of 1,2,4,8 (0001, 0010, 0100, 1000)
    ...
    if maze_array[x][y] == 1 or maze_array[x][y] in (int(MAZE_HEIGHT/8), int(MAZE_WIDTH/8)) or maze_array[x][y] in (int(MAZE_HEIGHT/4), int(MAZE_WIDTH/4)) or maze_array[x][y] in (int(MAZE_HEIGHT/2), int(MAZE_WIDTH/2)) or maze_array[x][y] == MAZE_HEIGHT or MAZE_WIDTH:
    ...
    while True:
        ...
        if 7 <= current_x <= 8 and 7 <= current_y <= 8:
            ...
            if state is State.final_run:
                final_score = total_score * 1 + best_run_score #as per score formua from rules
        ...
        while (current_x != goto_x or current_y != goto_y):
            if state is State.start_to_goal or state is State.goal_to_start:
                ...
                if encountered_wall == False:
                    if not all([all([((cell == 0) != (cell == MAZE_WIDTH)) for cell in row[1:-1]]) for row in maze_array[1:-1]]): #checks if there are walls in array; != is xor for two booleans
